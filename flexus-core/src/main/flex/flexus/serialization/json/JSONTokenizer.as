	...
	* the character to the next character after the token
	...
	public function getNextToken():JSONToken
	{
		...
		switch (ch)
		{
			...
			case 'f': // attempt to read false
				var possibleFalse:String = "f" + nextChar() + nextChar() + nextChar() +
					nextChar();
	...
	public function unescapeString(input:String):String
	{
		...
		do
		{
			...
			if (backslashIndex >= 0)
			{
				result += input.substr(nextSubstringStartPosition, backslashIndex -
									   nextSubstringStartPosition);
				...
				switch (escapedChar)
				{
					...
					case 'u':
						...
						for (var i:int = nextSubstringStartPosition; i < nextSubstringStartPosition +
							4; i++)
						{
							...
							{
								parseError("Excepted a hex digit, but found: " +
										   possibleHexChar);
							}
