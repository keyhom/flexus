...
public class URI
{
	...
	static public const CHILD:int = 1;

	//----------------------------------
	// EQUAL 
	//----------------------------------

	static public const EQUAL:int = 2;
	...
	static public const NOT_RELATED:int = 0;

	//----------------------------------
	// PARENT 
	//----------------------------------

	static public const PARENT:int = 3;
	...
	static public const UNKNOWN_SCHEME:String = "unknown";
	...
	static public const URIbaselineEscape:String = URImustEscape + ":?#/@";
	...
	static public const URImustEscape:String = " %";
	...
	static public const URInonHierEscape:String =  URImustEscape + "?#/";
	...
	static public const URIpathEscape:String = URImustEscape + "?#";
	...
	static public const URIqueryEscape:String = URImustEscape + "#";
	...
	// syntax.
	static public const URIqueryPartEscape:String = URImustEscape + "#&=";

	//----------------------------------
	// resolver 
	//----------------------------------

	static protected var _resolver:IURIResolver = null;
	...
	static protected const URIauthorityExcludedBitmap:URIEncodingBitmap = URIbaselineExcludedBitmap;
	...
	// this.
	static protected const URIbaselineExcludedBitmap:URIEncodingBitmap = new URIEncodingBitmap(URIbaselineEscape);
	...
	// uses too.
	static protected const URIfragmentExcludedBitmap:URIEncodingBitmap = URIqueryExcludedBitmap;

	//----------------------------------
	// URInonHierexcludedBitmap 
	//----------------------------------

	// Characters that need to be escaped in the non-hierarchical part
	static protected const URInonHierexcludedBitmap:URIEncodingBitmap = new URIEncodingBitmap(URInonHierEscape);

	//----------------------------------
	// URIpathExcludedBitmap 
	//----------------------------------

	// Path escaping bitmap
	static protected const URIpathExcludedBitmap:URIEncodingBitmap = new URIEncodingBitmap(URIpathEscape);

	//----------------------------------
	// URIportExludedBitmap 
	//----------------------------------

	// Port escaping bitmap
	static protected const URIportExludedBitmap:URIEncodingBitmap = URIbaselineExcludedBitmap;
	...
	static protected const URIqueryExcludedBitmap:URIEncodingBitmap = new URIEncodingBitmap(URIqueryEscape);

	//----------------------------------
	// URIqueryPartExcludedBitmap 
	//----------------------------------

	// Query (individual parts) escaping bitmap
	static protected const URIqueryPartExcludedBitmap:URIEncodingBitmap = new URIEncodingBitmap(URIqueryPartEscape);

	//----------------------------------
	// URIschemeExcludedBitmap 
	//----------------------------------

	// Scheme escaping bitmap
	static protected const URIschemeExcludedBitmap:URIEncodingBitmap = URIbaselineExcludedBitmap;

	//----------------------------------
	// URIuserpassExcludedBitmap 
	//----------------------------------

	// User/pass escaping bitmap
	static protected const URIuserpassExcludedBitmap:URIEncodingBitmap = URIbaselineExcludedBitmap;
	 ...
	 */
	static protected function compareStr(str1:String, str2:String, sensitive:Boolean =
										 true):Boolean
	{
		if (sensitive == false)
		{
	...
	public function getCommonParent(uri:URI, caseSensitive:Boolean = true):URI
	{
		...
		if (!thisURI.isAbsolute() || !thatURI.isAbsolute() || thisURI.isHierarchical() ==
			false || thatURI.isHierarchical() == false)
		{
	...
	public function getRelation(uri:URI, caseSensitive:Boolean = true):int
	{
		...
		else if (thisURI.isHierarchical() == false || thatURI.isHierarchical() ==
			false)
		{
			// One or both of the URI's are non-hierarchical.
			if (((thisURI.isHierarchical() == false) && (thatURI.isHierarchical() ==
				true)) || ((thisURI.isHierarchical() == true) && (thatURI.isHierarchical() ==
				false)))
			{
	...
	public function makeRelativeURI(base_uri:URI, caseSensitive:Boolean = true):Boolean
	{
		...
		if ((isOfType(base_uri.scheme) == false) || (this.authority != base_uri.
			authority))
		{
	 ...
	 */
	public function setParts(schemeStr:String, authorityStr:String, portStr:String,
							 pathStr:String, queryStr:String, fragmentStr:String):void
	{
	...
	public function unknownToURI(unknown:String, defaultScheme:String = "http"):Boolean
	{
		...
		else
		{
			...
			if (path == ".." || path == "." || (path.length >= 3 && path.substr(0,
																				3) ==
	...
	protected function parseURI(uri:String):Boolean
	{
		...
		if (index != -1)
		{
			...
			if (baseURI.length > (index + 1)) // +1 is to skip the '#'
				_fragment = baseURI.substr(index + 1, baseURI.length - (index +
										   1));
		...
		if (index != -1)
		{
			if (baseURI.length > (index + 1))
				_query = baseURI.substr(index + 1, baseURI.length - (index +
										1)); // +1 is to skip the '?'
