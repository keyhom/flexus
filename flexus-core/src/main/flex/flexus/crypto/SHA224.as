...
public class SHA224 {
    ...
    static private function hashBlocks(blocks:Array):ByteArray {
        ...
        var k:Array = new Array(0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
        ...
        for (var i:int = 0; i < len; i += 16) {
            ...
            for (var t:int = 0; t < 64; t++) {
                ...
                if (t < 16) {
                    ...
                    var ws0:int = IntUtil.ror(w[t - 15], 7) ^ IntUtil.ror(w[t -
                            15],
                            18) ^
                            (w[t - 15] >>> 3);
                    var ws1:int = IntUtil.ror(w[t - 2], 17) ^ IntUtil.ror(w[t -
                            2],
                            19) ^
                            (w[t - 2] >>> 10);
                ...
                var s0:int = IntUtil.ror(a, 2) ^ IntUtil.ror(a, 13) ^ IntUtil.
                        ror(a, 22);
                var maj:int = (a & b) ^ (a & c) ^ (b & c);
                var t2:int = s0 + maj;
                var s1:int = IntUtil.ror(e, 6) ^ IntUtil.ror(e, 11) ^ IntUtil.
                        ror(e, 25);
                var ch:int = (e & f) ^ ((~e) & g);
